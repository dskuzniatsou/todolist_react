...
test('correct task should change its status', () => {
    ...
    expect(endState.todolistId2[1].isDone).toBe(false) // была true, стала false
...
test('correct task should change its title', () => {
    ...
    expect(endState.todolistId2[1].title).toBe('coffee') // было 'milk', стало 'coffee'
    expect(endState.todolistId2[1].isDone).toBe(true) // статус не должен измениться
})
